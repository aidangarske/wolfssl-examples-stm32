/* hw_init.c - STM32H563ZI (NUCLEO-H563ZI), bare-metal CMSIS only
 *
 * Copyright (C) 2026 wolfSSL Inc.
 *
 * Direct-register board init for NUCLEO-H563ZI:
 *   - HSI 64 MHz as SYSCLK (no PLL); flash latency 1WS at default VOS
 *   - USART3 on PD8 (TX) / PD9 (RX) AF7, 115200 8N1, routed to ST-LINK VCP
 *   - Crypto: HASH and RNG clock-enable via wolfSSL macros (no AES on H563)
 *
...
static void clock_init(void)
{
     ...
     * Raise the core voltage to VOS0 (highest range) first: at the
     * reset-default low VOS, 64 MHz with a single wait state is under-
     * specified and causes intermittent flash instruction-fetch bus
     * errors (IBUSERR) once non-trivial code runs. VOS0 puts 64 MHz
     * comfortably in spec. */
    PWR->VOSCR = (PWR->VOSCR & ~PWR_VOSCR_VOS_Msk) | PWR_VOSCR_VOS_Msk; /* VOS0 */
    while ((PWR->VOSSR & PWR_VOSSR_VOSRDY) == 0u) { }
    /* Set 2 wait states for flash at 64 MHz / VOS0 (1WS minimum; +1 margin). */
    FLASH->ACR = (FLASH->ACR & ~FLASH_ACR_LATENCY_Msk) | (2u << FLASH_ACR_LATENCY_Pos);
