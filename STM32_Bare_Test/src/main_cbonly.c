 ...
 *
 * Crypto-callback-only test app for STM32_Bare_Test. Runs the four primitives
 * an embedded TLS/secure-boot stack needs -- ECDSA (sign/verify), AES-GCM
 * (full payload), HMAC-SHA256 and TRNG -- entirely on hardware through the
 * wolfCrypt crypto-callback framework (the STM32 DHUK device, see
 * wc_Stm32_DhukRegister). Built with the STM32_BARE_CB_ONLY preset, the
 * software crypto is stripped so flash shrinks; the primitives run on the
 * SAES / PKA / HASH / RNG blocks with keys that never enter software.
 *
 *   [1] ECDSA sign via a DHUK-wrapped scalar (PKA), verified with the public
 *       key. Asserts dhuk_is_ccb on a CCB build (no software keygen fallback).
 ...
 *       (AES_GCM_AUTH_E). Exercises the SAES CTR + GHASH path.
 *   [3] HMAC-SHA256 via a DHUK-devId Hmac (STM32 HASH block), matched against
 *       the RFC 4231 test-case-1 vector.
 *   [4] TRNG via a DHUK-devId RNG (STM32 hardware RNG through the callback).
 *
 * A backend that is gated off or cannot complete on TZEN=0 silicon
 * (CRYPTOCB_UNAVAILABLE / a secure-context timeout) is reported as an expected
 * soft-PASS for the asymmetric legs, not a failure. Build with:
 *   make BOARD=u3 CONFIG=bare TARGET=cbonly
 */
...
/* Debugger-readable result sink (magic 0xCB010001 once main() completes), so
 ...
 * counterpart. Mirrors the transparent DHUK sign path: make a P-256 keypair,
 * wrap its scalar with the DHUK-derived key, import the wrapped scalar + seed,
...
int main(void)
{
    ...
    printf("\n");
    printf("========================================\n");
    printf("wolfCrypt callback-only test - %s (CONFIG=%s)\n",
...
#else
    printf("callback-only not enabled in this build (need WOLFSSL_DHUK + "
